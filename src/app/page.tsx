...
export default function HomePage() {
  ...
  const Pill = ({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) => (
    <button onClick={onClick} className={`text-[12px] font-semibold px-3.5 py-1.5 rounded-full border-none cursor-pointer transition-all font-sans ${active ? "bg-ocean-900 text-white" : "bg-white text-ocean-600 shadow-[0_0_0_1px_rgba(24,95,165,0.15)] hover:shadow-[0_0_0_1px_#FF6B6B]"}`}>
      {label}
    </button>
  );
      ...
      <div className="bg-ocean-900 px-6 pt-16 pb-14">
        <div className="max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-coral/10 border border-coral/25 rounded-full px-3.5 py-1.5 mb-5">
            ...
            <span className="text-[11px] text-coral font-bold font-mono tracking-widest uppercase">Live on Sui Network</span>
          </div>
